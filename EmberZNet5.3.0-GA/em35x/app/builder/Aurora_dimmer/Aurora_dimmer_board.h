 ...
 * - PA2 - SC2SCLK
 * - PA3 - SC2nSSEL
 * - PA4 - PTI_EN - STM32 NRST pin
 * - PA5 - PTI_DATA
 * - PA6 - LED (on RCM), or Radio HoldOff
 * - PA7 - LED (on RCM)
 * - PB0 - Power Amplifier shutdown control - STM32 BOOT0 pin
 * - PB1 - SC1TXD
 * - PB2 - SC1RXD
 * - PB3 - 
 * - PB4 - 
 * - PB5 - Host CTS active hi (host asserts this line hi when it ready to receive data from module)
...
enum HalBoardLedPins {
    BOARDLED0 = PORTB_PIN(3),
    BOARDLED1 = PORTB_PIN(4),
    LED_GREEN = BOARDLED0,
    LED_RED = BOARDLED1,
    BOARD_ACTIVITY_LED  = INVALID_LED,

    STM32_RESET = PORTA_PIN(4),
    STM32_BOOT0 = PORTB_PIN(0)
};
...
/*
Host Uart Ready definitions (this pin is asserted hi by host when it is ready to receive data from module)
*/
#define HOST_UART_READY           PORTB_PIN(5)
#define HOST_UART_READY_IN        GPIO_PBIN
...
int8u gpioOutPowerUp[3] = {                                               \
                           ((0                  <<PA0_BIT)|               \
                            ...
                            (1                  <<PA3_BIT)|               \
                            (0                  <<PA4_BIT)|  /* STM32 Reset OP active hi */      \
                            (1                  <<PA5_BIT)|               \
                            (0                  <<PA6_BIT)|               \
                            (0                  <<PA7_BIT)),              \
                           ((0                  <<PB0_BIT)|  /* STM32 Bootload OP active hi */   \
                            (1                  <<PB1_BIT)|  /* SC1TXD  */\
                            (0                  <<PB2_BIT)|  /* SC1RXD  */\
                            (1                  <<PB3_BIT)|               \
                            (0                  <<PB4_BIT)|               \
                            (0                  <<PB5_BIT)|  /* Host CTS IP active hi */         \
                            (1                  <<PB6_BIT)|               \
                            (0                  <<PB7_BIT)),              \
                           ((1                  <<PC0_BIT)|               \
                            (1                  <<PC1_BIT)|               \
...
int8u gpioOutPowerDown[3] = {                                               \
                             ...
                             ((0                  <<PB0_BIT)|  /* STM32 Bootload OP active hi */   \
                              ...
                              (GPIOOUT_PULLDOWN   <<PB5_BIT)|  /* Host CTS IP active hi */ \
