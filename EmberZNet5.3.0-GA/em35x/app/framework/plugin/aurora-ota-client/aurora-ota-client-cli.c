...
#ifdef EMBER_AF_PLUGIN_AURORA_OTA_CLIENT
...
void otaCliVerify(void)
{
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_SIGNATURE_VERIFICATION_SUPPORT)
  int8u index = (int8u)emberUnsignedCommandArgument(0);
  EmberAfOtaImageId id = emAfOtaFindImageIdByIndex(index);
  if (!emberAfIsOtaImageIdValid(&id)) {
    otaPrintln("Error: No image at index %d", index);
...
void otaPrintClientInfo(void)
{
  ...
  otaPrintln("Download Delay ms:         %l", (int32u)EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_DELAY_MS);
  otaPrintln("Run Upgrade Delay ms:      %l", (int32u)EMBER_AF_RUN_UPGRADE_REQUEST_DELAY_MS);
  emberAfCoreFlush();
  otaPrintln("Verify Delay ms:           %l", (int32u)EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_VERIFY_DELAY_MS);
  otaPrintln("Download Error Threshold:  %d",    EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_ERROR_THRESHOLD);
  otaPrintln("Upgrade Wait Threshold:    %d",    EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_UPGRADE_WAIT_THRESHOLD);
  emberAfCoreFlush();
  ...
  otaPrintln("Page Request Size: %d bytes", 
             EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_PAGE_REQUEST_SIZE);
  otaPrintln("Page Request Timeout: %d sec.", 
             EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_PAGE_REQUEST_TIMEOUT_SECONDS);
#endif
...
#endif //EMBER_AF_PLUGIN_AURORA_OTA_CLIENT
