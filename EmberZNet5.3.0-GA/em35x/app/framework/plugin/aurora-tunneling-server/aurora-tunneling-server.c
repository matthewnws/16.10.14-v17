...
#include "aurora-tunneling-server.h"
...
static EmAfTunnelingServerTunnel tunnels[EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_TUNNEL_LIMIT];
...
void emberAfTunnelingClusterServerInitCallback(int8u endpoint)
{
  ...
  int16u closeTunnelTimeout = EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_CLOSE_TUNNEL_TIMEOUT;
  int8u i;

  for (i = 0; i < EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_TUNNEL_LIMIT; i++) {
                                                     ...
                                                     int16u maximumIncomingTransferSize)
{
  ...
  if (!emAfIsTestProtocol(protocolId, manufacturerCode)
      && !emberAfPluginAuroraTunnelingServerIsProtocolSupportedCallback(protocolId,
                                                                  manufacturerCode)) 
  {
  ...
  if (status == EMBER_ZCL_TUNNELING_TUNNEL_STATUS_SUCCESS) {
    emberAfPluginAuroraTunnelingServerTunnelOpenedCallback(tunnelId,
                                                     protocolId,
                                                     manufacturerCode,
                                                     flowControlSupport,
                                                     maximumIncomingTransferSize);
  ...
  emberAfFillCommandTunnelingClusterRequestTunnelResponse(tunnelId,
                                                          status,
                                                          EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_MAXIMUM_INCOMING_TRANSFER_SIZE);
...
boolean emberAfTunnelingClusterCloseTunnelCallback(int16u tunnelId)
{
  ...
  if (status == EMBER_ZCL_STATUS_SUCCESS) 
  {
    ...
    emberAfPluginAuroraTunnelingServerTunnelClosedCallback(tunnelId,
                                                                  ...
                                                                  int8u* data)
{
  ...
  if (status == EMBER_ZCL_STATUS_SUCCESS) 
  {
    if (dataLen <= EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_MAXIMUM_INCOMING_TRANSFER_SIZE) 
    {
      ...
      {
        emberAfPluginAuroraTunnelingServerDataReceivedCallback(tunnelId, data, dataLen);
      }
      emberAfSendImmediateDefaultResponse(status);
      return TRUE;
    }
                                                                       ...
                                                                       int8u transferDataStatus)
{
  ...
  if (status == EMBER_ZCL_STATUS_SUCCESS) 
  {
    emberAfPluginAuroraTunnelingServerDataErrorCallback(tunnelId,
...
EmberAfStatus emberAfPluginTunnelingServerTransferData(int16u tunnelId,
                                                       ...
                                                       int16u dataLen)
{
  if (tunnelId < EMBER_AF_PLUGIN_AURORA_TUNNELING_SERVER_TUNNEL_LIMIT
...
#endif
          tunnels[i].clientEndpoint = UNUSED_ENDPOINT_ID;
          emberAfPluginAuroraTunnelingServerTunnelClosedCallback(i,
