...
#include "aurora-on-off.h"

#ifdef EMBER_AF_PLUGIN_AURORA_LEVEL_CONTROL
  #include "../aurora-level-control/aurora-level-control.h"
#endif //EMBER_AF_PLUGIN_AURORA_LEVEL_CONTROL
                                                  ...
                                                  boolean initiatedByLevelChange)
{
  ...
  //GB: Dont need this here - level control tick will pick up the new current level attr value
  // and change the level/colour  emberAfOnOffClusterSetNewValueCallback(newValue);
...
#elif defined(EMBER_AF_PLUGIN_AURORA_LEVEL_CONTROL)
