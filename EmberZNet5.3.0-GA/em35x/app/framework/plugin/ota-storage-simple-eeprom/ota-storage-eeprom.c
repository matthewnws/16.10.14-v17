...
EmberCommandEntry emberAfPluginOtaStorageSimpleEepromCommands[] = {
 ...
 || defined(EMBER_AF_PLUGIN_TELEGESIS_OTA_STORAGE_SIMPLE_EEPROM_ENABLE_SOC_APP_BOOTLOADER_COMPATIBILITY_MODE)
...
boolean emberAfOtaStorageDriverInitCallback(void)  //GB
{   
...
{
  extern void emAfEepromInfoCommand(void);
...
//GB
boolean isEepromAvailable(void) 
{
    static boolean eepromAvailable = FALSE;
    static boolean initialised = FALSE;

    if (!initialised) 
    {
        if (eepromInit() == EEPROM_SUCCESS) 
        {
            emberAfDebugPrintln("isEepromAvailable: Eeprom available");
            eepromAvailable = TRUE;
        } 
        else 
        {
            emberAfDebugPrintln("isEepromAvailable: Eeprom not available");
