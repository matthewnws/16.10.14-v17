...
#include "app/framework/plugin/aurora-host-protocol/aurora-host-bootload.h"
#include "aurora-ota-client-policy.h"
...
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_INCLUDE_HARDWARE_VERSION)
  #define HARDWARE_VERSION EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_HARDWARE_VERSION
#else
                                         ...
                                         int16u* hardwareVersion)
{
  ...
  currentImageInfo->imageTypeId     = EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_IMAGE_TYPE_ID2;
  currentImageInfo->firmwareVersion = EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_FIRMWARE_VERSION2;
                                                              ...
                                                              const EmberAfOtaImageId* id)
{
...
#if !defined(EZSP_HOST) && defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_EBL_VERIFICATION)
  int16u pages;
  ...
  if (pages == BL_IMAGE_IS_VALID_CONTINUE) {
    ...
    // --- RN was here. Added a check to make sure STM32 OTA files are skipped
    int32u offset;
    int32u endOffset;
    if (EMBER_AF_OTA_STORAGE_SUCCESS != emAfOtaStorageGetTagOffsetAndSize(id, OTA_TAG_AURORA_STM32_IMAGE, &offset, &endOffset))
    {
                                                 ...
                                                 const EmberAfOtaImageId* id)
{
  ...
  if (result != EMBER_AF_OTA_DOWNLOAD_AND_VERIFY_SUCCESS) {
    ...
    emAfOtaPrintAllImages();
    
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_POLICY_DELETE_FAILED_DOWNLOADS)
...
void emberAfOtaClientBootloadCallback(const EmberAfOtaImageId* id)
{
    ...
    {
        otaPrintln("Image contains an Upgrade Image Tag (0x%2X) (0x%4X) (0x%4X).", OTA_TAG_AURORA_STM32_IMAGE, offset, endOffset);
        
        // --- RN was here. This is where the STM32 OTA is done
        if(emberAfPluginAuroraBootloadUpgrade(offset, endOffset))
          emberAfDebugPrint("STM32 bootload successfully completed\r\n");

        else 
          emberAfDebugPrint("STM32 bootload failed!\r\n");
        
        // Erase the EEPROM
        emberAfOtaStorageDriverInvalidateImageCallback();
        
        // Reset all devices on the STM32 reset line
        STM32ToggleReset();
    }
