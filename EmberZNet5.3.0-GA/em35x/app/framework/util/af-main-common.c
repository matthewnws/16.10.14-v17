                                                     ...
                                                     EmberAfMessageSentFunction callback)
{
    ...
    boolean bindingSentOk = FALSE;  //GB result flag.
    
    for (int8u i=0; i<EMBER_BINDING_TABLE_SIZE; i++) 
    {
        EmberBindingTableEntry binding;
        status = emberGetBinding(i, &binding);    
        if (status != EMBER_SUCCESS) 
        {
            continue;  //GB dont return
        }
            ...
            && binding.clusterId == apsFrame->clusterId) 
        {
            ...
            if (status == EMBER_SUCCESS) 
            {
                bindingSentOk = TRUE;  //GB Set flag, dont return. 
            }
        }
    }

    //GB Set status for return- 'SUCCESS' if at least one msg has been sucessfully sent to a binding entry.
    status = (bindingSentOk) ? EMBER_SUCCESS : EMBER_INVALID_BINDING_INDEX;
